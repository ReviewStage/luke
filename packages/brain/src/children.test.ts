...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() => settle());
      assert.equal((yield* h.agent.waitAsk(runId, 1))?.status, BRAIN_REQUEST_STATUS.SUCCEEDED);
      ...
      const opened = yield* h.agent.deliverChildCompletion(
        ...childCompletion({
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const record = yield* ask(h, "delegate this");
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      assert.equal((yield* h.agent.waitAsk(runId, 1))?.status, BRAIN_REQUEST_STATUS.FAILED);
      // The retry opens its own turn and lands.
      inner.answers.push(answered([message("")]));
      const retried = yield* h.agent.deliverChildCompletion(...completion);
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      open();
      assert.deepEqual(yield* Fiber.join(pending), { delivered: true });
      assert.equal((yield* h.agent.waitAsk(runId, 1))?.status, BRAIN_REQUEST_STATUS.FAILED);
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const failed = yield* h.agent.deliverChildCompletion(
        ...childCompletion({
      ...
      h.repository.refuse();
      const refused = yield* h.agent.deliverChildCompletion(
        ...childCompletion({
