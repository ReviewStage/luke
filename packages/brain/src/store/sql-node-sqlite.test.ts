...
import { Context, Data, Deferred, Effect, Exit, Fiber, Layer, Result, Schema, Scope } from "effect";
      ...
      Effect.gen(function* () {
        ...
        yield* sql.withTransaction(
          Effect.gen(function* () {
            ...
            const inner = yield* Effect.result(
              sql.withTransaction(
                Effect.gen(function* () {
  ...
  it.effect("a transaction holds the connection, so another fiber's statement waits for it", () =>
    withNodeFileSystem(
      Effect.gen(function* () {
        ...
        assert.equal(outsider.pollUnsafe(), undefined);
      ...
      withNodeFileSystem(
        Effect.gen(function* () {
          ...
          if (Result.isFailure(refused)) {
            ...
            assert.equal(readErrorCode(refused.failure.reason.cause).errcode, SQLITE_BUSY);
          }
  ...
  it.effect("binds the kinds SQLite takes and refuses one it does not", () =>
    withNodeFileSystem(
      Effect.gen(function* () {
        ...
        const refused = yield* Effect.result(sql`INSERT INTO kinds (i) VALUES (${true})`);
    ...
    () =>
      withNodeFileSystem(
        Effect.gen(function* () {
          ...
          const refused = yield* Effect.result(sql`SELECT ${large} AS big`);
