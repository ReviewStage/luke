...
const TURN_LINES: readonly string[] = [
  ...
  `agent's recent transcript in full), then either call ${BRAIN_TOOL.ANNOUNCE} once, covering`,
  "every agent worth mentioning in one breath, or do nothing. Text you write in this kind of",
  "turn is not spoken; only the briefing is. There is no floor: a finished, waiting, blocked, or",
  "errored agent is news only when the persona's own rule says it is, and a developer who is",
  "told about every stop will stop listening. No act runs in this kind of turn; the only tools",
  "offered are reading a transcript and announcing.",
  "",
...
const TOOL_LINES: readonly string[] = [
  "The tools.",
  "",
  "Acts are offered only in a developer-ask turn; observed-events and hold-released turns are",
  `offered ${BRAIN_TOOL.READ_TRANSCRIPT} and ${BRAIN_TOOL.ANNOUNCE} alone. Name a session only by the provider_id and`,
