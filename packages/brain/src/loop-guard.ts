 ...
 * repeat, the ping-pong between two signatures, the unknown-tool repeat, and
 * the global circuit breaker, with the pinned thresholds; the pinned poll and
 * argument-churn detectors name OpenClaw's own tools and are left out.
 */
...
export function hashToolCall(name: string, argumentsJson: string): string {
  return `${name}:${stableStringify(parsedArguments(argumentsJson))}`;
}
...
function noProgressStreak(
  ...
  for (let index = history.length - 1; index >= 0; index -= 1) {
    ...
    if (!record || record.name !== name || record.argsHash !== argsHash) break;
    if (!record.resultHash) break;
    if (latest === undefined) latest = record.resultHash;
    else if (latest !== record.resultHash) break;
    count += 1;
  }
...
export class LoopGuard {
  ...
  record(invocation: ToolInvocation, result: ToolResult): void {
    ...
    this.#history.push({
      ...
      argsHash: hashToolCall(invocation.name, invocation.argumentsJson),
      resultHash: stableStringify(parsedArguments(result.outputJson)),
