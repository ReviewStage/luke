...
it.effect("stores a key as a bearer-authenticated POST and reads the confirmation", () =>
  Effect.gen(function* () {
    ...
    const answer = yield* client(api.layer).storeKey(
...
it.effect("a key the service would refuse by shape never travels", () =>
  Effect.gen(function* () {
    ...
    assert.equal(yield* vault.storeKey(CLOUD_AGENT_PROVIDER_ID.CONDUCTOR, ""), undefined);
    assert.equal(
      yield* vault.storeKey(CLOUD_AGENT_PROVIDER_ID.CONDUCTOR, "key with spaces"),
      undefined,
    );
    assert.equal(
      yield* vault.storeKey(CLOUD_AGENT_PROVIDER_ID.CONDUCTOR, "k".repeat(513)),
      undefined,
    );
...
it.effect("lists stored entries without a body and validates the answer", () =>
  Effect.gen(function* () {
    ...
    const keys = yield* client(api.layer).listKeys();
...
it.effect("deletes one provider's key and reads whether one was removed", () =>
  Effect.gen(function* () {
    ...
    const answer = yield* client(api.layer).deleteKey(CLOUD_AGENT_PROVIDER_ID.CONDUCTOR);
...
it.effect("a refusal and an answer the vault contract does not admit both read as no answer", () =>
  Effect.gen(function* () {
    ...
    assert.equal(
      yield* client(refused.layer).storeKey(CLOUD_AGENT_PROVIDER_ID.CONDUCTOR, "key_1234"),
      undefined,
    );
    ...
    assert.equal(yield* client(malformed.layer).listKeys(), undefined);
  }),
);
