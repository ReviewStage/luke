...
function observerOf(
  ...
  return { provider, observe: () => Effect.succeed(observations) };
}
    ...
    observe: () =>
      Effect.fail(
        new SessionProviderObservationError({
          ...
          message: "session state is unreadable",
          cause: new Error("session state is unreadable"),
        }),
      ),
  };
}
...
test("leaves each half of a provider saying where its own sessions run", async () => {
  ...
  await runEffect(registry.refresh(adapter));
...
test("keeps a session two observers both reached from being reported twice", async () => {
  ...
  const registry = new InMemorySessionRegistry();
  assert.equal((await runEffect(registry.refresh(adapter))).sessions.length, 1);
});
...
test("fails the pass when one observer fails, rather than retiring its sessions", async () => {
  ...
  await assert.rejects(() => runEffect(adapter.observe()), /unreadable/);
  await assert.rejects(() => runEffect(registry.refresh(adapter)), /unreadable/);
  assert.deepEqual(
...
function messenger(
  ...
  return {
    ...
    sendMessage: (message) => Effect.succeed(answer(message)),
  };
}
...
function workspaceCreator(
  ...
  return {
    ...
    createWorkspace: (request) => Effect.succeed(answer(request)),
  };
}
...
test("offers every observer's projects and carries a creation ask to the one that offered it", async () => {
  ...
  const result = await runEffect(adapter.createWorkspace({ providerProjectId: "proj-2" }));
...
test("answers unsupported when no observer offers workspace creation", async () => {
  ...
  assert.deepEqual(adapter.workspaceProjects(), []);
  assert.deepEqual(await runEffect(adapter.createWorkspace({ providerProjectId: "proj-1" })), {
    status: PROVIDER_ACT_RESULT_STATUS.UNSUPPORTED,
  });
});
