...
/**
 * Every kind of action Luke can carry against a session or the workspace around
 ...
 * way where an action is offered and another where it is checked.
 */
...
export interface AdvertisedControl {
  kind: typeof ACTION_KIND.CONTROL;
  id: string;
  label: string;
...
/**
 * One action a session's provider documents for it now. Each entry carries what
 * that action needs and nothing else, so a target no action uses cannot ride along
 ...
 */
export function advertisedActionFor<Kind extends AdvertisedActionKind>(
  session: Pick<SessionFields, "advertises">,
  kind: Kind,
): AdvertisedActionOf<Kind> | undefined {
  return session.advertises?.find(
    (advertised): advertised is AdvertisedActionOf<Kind> => advertised.kind === kind,
  );
}
...
export function advertisedControls(
  ...
  return (
    session.advertises?.filter(
      (advertised): advertised is AdvertisedControl => advertised.kind === ACTION_KIND.CONTROL,
    ) ?? []
  );
}
