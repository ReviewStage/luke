...
/** The day a dated note's name is about when that day lies inside the light lookback; nothing for any other file. */
export function recentDailyNoteDay(name: string, now: number): string | undefined {
  const day = parseDailyNoteName(name)?.day;
...
export function noteSeeds(files: readonly NoteFile[], now: number): CandidateSeed[] {
  ...
  for (const file of [...files].sort((a, b) => a.name.localeCompare(b.name))) {
    const day = recentDailyNoteDay(file.name, now);
    if (!day) continue;
