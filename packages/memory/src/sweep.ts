...
import { prepareForIngestion, privateKeyLines } from "./redaction.js";
  ...
  if (params.modelStood) {
    ...
    if (plan && !rejection) {
      ...
      if (result) deepPath = DEEP_PATH.MODEL_PLAN;
        ...
        notes.push("rewrite rejected: it would lose too many prior entries or exceed the budget");
    } else if (rejection) {
...
/** The dated notes inside the lookback, read whole; any other file in the directory is left unread. */
async function readNoteFiles(workspaceDirectory: string, now: number): Promise<NoteFile[]> {
  ...
  for (const name of names.filter((candidate) => recentDailyNoteDay(candidate, now))) {
    try {
...
export async function runConsolidationSweep(
  ...
  try {
    ...
    const noteCandidates = noteSeeds(
      await readNoteFiles(dependencies.workspaceDirectory(), now),
      now,
