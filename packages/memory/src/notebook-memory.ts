...
export const makeNotebookMemory = /* @__PURE__ */ Effect.fn("makeNotebookMemory")(function* (
  options: NotebookMemoryOptions,
): Effect.fn.Return<NotebookMemory, never, FileSystem.FileSystem | Scope.Scope> {
  const fileSystem = yield* FileSystem.FileSystem;
  const scope = yield* Effect.scope;
  const agentId = options.agentId ?? DEFAULT_AGENT_ID;
  const standing = yield* Ref.make<RetrievalStanding>({
    mode: RETRIEVAL_MODE.KEYWORD_ONLY,
  });
  const passes = yield* Ref.make<Passes>({
    running: undefined,
    follow: undefined,
  });
  ...
  const embedQuery = /* @__PURE__ */ Effect.fnUntraced(function* (
    query: string,
    signal: AbortSignal,
  ): Effect.fn.Return<QueryEmbedding> {
    ...
    return {
      ...HYBRID,
      queryVector,
      identity: yield* Effect.orDie(identityOf(adapter)),
    };
  });
   ...
   */
  const search = /* @__PURE__ */ Effect.fnUntraced(function* (
    current: SessionKey,
    ask: {
      readonly query: string;
      readonly maxResults?: number;
  ...
  ): Effect.fn.Return<MemorySearchAnswer> {
  ...
  const embedAll = /* @__PURE__ */ Effect.fnUntraced(function* (
    adapter: EmbeddingAdapter,
    texts: readonly { hash: string; text: string }[],
  ): Effect.fn.Return<{ written: readonly EmbeddingWrite[]; failed?: string }, MemorySeamRefused> {
   ...
   */
  function runPass(deferred: Deferred.Deferred<MemorySyncReport | undefined>): Effect.Effect<void> {
  ...
  function forkPass(
    ...
    return Effect.asVoid(Effect.forkIn(runPass(deferred), scope));
  }
   ...
   */
  const requestSync = Effect.asVoid(Effect.forkIn(sync, scope));
  ...
  const watching = yield* Effect.cached(Effect.asVoid(Effect.forkIn(watchFiles, scope)));
