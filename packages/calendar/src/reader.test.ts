...
it.effect("every connected account is read, and their meetings stand apart", () =>
  Effect.gen(function* () {
...
it.effect("one bad account never blinds the others, and keeps what it last showed", () =>
  Effect.gen(function* () {
    const home: CalendarAccountCredential = {
      id: "home@example.com",
      refreshToken: "1//home-grant",
    ...
    workBroken = true;
    const second = yield* reader.observe();
...
it.effect("a calendar unread inside an OK answer is a failure, not a quieter calendar", () =>
  Effect.gen(function* () {
