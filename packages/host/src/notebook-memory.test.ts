...
test("a launch before any credential indexes keyword-only, and the first credentialed sync backfills the vectors without an edit", async (t) => {
  ...
  assert.equal((await h.store.ask("memory.status", {})).embeddedChunks, 0);
  ...
  const status = await h.store.ask("memory.status", {});
...
test("a transient embedding failure leaves keyword rows searchable and the next sync retries the vectors", async (t) => {
  ...
  await h.wiring.sync();
  const before = (await h.store.ask("memory.status", {})).embeddedChunks;
  assert.ok(before > 0);
  fs.writeFileSync(
  ...
  await h.wiring.sync();
  assert.equal(
    (await h.store.ask("memory.status", {})).embeddedChunks,
    before,
