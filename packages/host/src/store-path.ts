...
/**
 * Where the brain's store lives and where its worker's code is found, as pure
...
export function storeWorkerPath(bundleDirectory: string): string {
  const beside = path.join(bundleDirectory, STORE_WORKER_FILE);
