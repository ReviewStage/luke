...
it.effect(
  "the whole read renders the page as lines under the agent's roster name, and opens with the marker when history precedes it",
  () =>
    Effect.gen(function* () {
      const { reads, queries } = fixture([{ ...PAGE, hasOlder: true }, PAGE], NAMED_AGENT);

      const preceded = yield* reads.readTranscript(CLOUD);
      const whole = yield* reads.readTranscript(CLOUD);
...
it.effect("a session the roster names no agent for speaks under its provider's name", () =>
  Effect.gen(function* () {
...
it.effect(
  "the incremental read hands the cursor back as `after`, answers the newest consumed id, and says what was cut",
  () =>
    Effect.gen(function* () {
      ...
      const first = yield* reads.readTranscriptSince(CLOUD, undefined);
      const quiet = yield* reads.readTranscriptSince(CLOUD, "m-3");
      const more = yield* reads.readTranscriptSince(CLOUD, "m-3");
...
it.effect(
  "a local session is refused before any call, and a page the service could not answer is a refusal",
  () =>
    Effect.gen(function* () {
      const { reads, queries } = fixture([undefined, undefined]);

      assert.equal((yield* reads.readTranscript(LOCAL)).status, ACTION_RESULT_STATUS.UNSUPPORTED);
      assert.equal(
        (yield* reads.readTranscriptSince(LOCAL, undefined)).status,
        ACTION_RESULT_STATUS.UNSUPPORTED,
      );
      assert.equal(queries.length, 0);

      assert.equal((yield* reads.readTranscript(CLOUD)).status, ACTION_RESULT_STATUS.REJECTED);
      assert.equal(
        (yield* reads.readTranscriptSince(CLOUD, "m-3")).status,
        ACTION_RESULT_STATUS.REJECTED,
      );
    }),
);

it.effect("a page with no attributed words is not found rather than rendered empty", () =>
  Effect.gen(function* () {
