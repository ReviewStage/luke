...
test("a spoken panel ask opens a real tab and narrows only to what is observed", async () => {
  ...
  assert.equal((await show('{"tab":"about"}')).status, ACTION_RESULT_STATUS.REJECTED);
  ...
  assert.equal((await show('{"filters":["local"]}')).status, ACTION_RESULT_STATUS.REJECTED);
...
test("a spoken panel ask can combine filters, on the axes the chips combine on", async () => {
  ...
  assert.equal((await show('{"filters":[3]}')).status, ACTION_RESULT_STATUS.REJECTED);
  assert.equal((await show('{"filters":{"value":"cloud"}}')).status, ACTION_RESULT_STATUS.REJECTED);
...
test("a spoken panel ask can search, only where the list offers a search at all", async () => {
  ...
  assert.equal((await show('{"query":"parser"}', [])).status, ACTION_RESULT_STATUS.REJECTED);
});
...
test("a spoken panel ask can reorder the list in the panel's own two words", async () => {
  ...
  assert.equal((await show('{"sort":"alphabetical"}')).status, ACTION_RESULT_STATUS.REJECTED);
});
...
test("a spoken composer open takes only the two kinds, drafting only the developer's words", async () => {
  ...
  assert.equal((await open('{"kind":"complaint"}')).status, ACTION_RESULT_STATUS.REJECTED);
  assert.equal((await open('{"kind":""}')).status, ACTION_RESULT_STATUS.REJECTED);
  assert.equal((await open("{}")).status, ACTION_RESULT_STATUS.REJECTED);
  assert.equal((await open("not json")).status, ACTION_RESULT_STATUS.REJECTED);
});
...
test("a spoken update ask runs only the action the row's button offers", async () => {
  ...
  assert.equal(
    (await ask('{"action":"check"}', offersBrowser)).status,
