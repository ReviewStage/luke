...
/**
 * The acts Conductor documents, each as the route it takes and the write that
 * carries it. Every target here arrived from the observation `dispatchAct`
...
export const CONDUCTOR_WRITE_ROUTE = {
  ...
  archiveWorkspace: (workspaceId: string): CloudWriteRoute => ({
    ...
    // up mid-act and an archive that landed is reported as one that may not
    // have.
...
/**
 * The credential is read at act time, not held from the observation pass, so
 ...
 * the session advertised the act while a key stood behind it, and a key that
 * has since gone is a different fact than a session that moved on.
 */
const MISSING_KEY: ProviderActResult = {
  status: ACT_RESULT_STATUS.REJECTED,
  reason: `${CONDUCTOR_PROVIDER_NAME}'s API key is no longer configured.`,
};

/** One documented write, under the key as it stands at the moment of the act. */
...
export function conductorActs(pass: CloudPass): ActHandlers {
  return {
...
async function createWorkspace(
  pass: CloudPass,
  input: Parameters<ActHandlers["createWorkspace"]>[0],
  ...
  if (created.outcome.status !== ACT_RESULT_STATUS.ACCEPTED) return created.outcome;
  ...
  if (delivered.outcome.status === ACT_RESULT_STATUS.ACCEPTED) return landed;
  return {
    status: ACT_RESULT_STATUS.REJECTED,
