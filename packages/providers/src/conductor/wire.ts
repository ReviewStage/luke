 ...
 * the document's text but session ids the same roster reported, each
 * validated as a UUID first, so no name, title, or message a provider
 * controls can ever be spliced into it.
 *
 * It is the observation pass's, and asks for the agent kind and nothing
 * else. The view also holds each chat's transcript, and no column of it is
 * named: the conversation is the documented messages endpoint's to read, at
 * the developer's own press or the brain's own reads, never a pass's. The
 * scheduled opener sends no document at all; which chats moved is answered
 * from the status instants the pass already read.
 */
