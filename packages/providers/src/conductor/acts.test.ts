...
import { CONDUCTOR_WRITE_ROUTE } from "./acts.js";
...
test("asks the slow deadline for an archive, whose answer waits on the workspace standing down", () => {
  ...
  // deadline shorter than the act gives up mid-write and reports an archive
...
test("creates a workspace through Conductor's documented creation endpoint", async () => {
  ...
  const unnamed = await dispatchAct(
    plugin,
    "createWorkspace",
...
test("a chosen agent and model ride the creation, and an unlisted pairing does not", async () => {
  ...
  const chosen = await dispatchAct(
    plugin,
    "createWorkspace",
    admittedForTest({
      providerProjectId: LUKE_PROJECT.id,
  ...
  const effortless = await dispatchAct(
    plugin,
    "createWorkspace",
    admittedForTest({
      providerProjectId: LUKE_PROJECT.id,
  ...
  for (const agentSelection of [
    ...
    const unlisted = await dispatchAct(
      plugin,
      "createWorkspace",
      admittedForTest({
        providerProjectId: LUKE_PROJECT.id,
...
test("a stored model rides a new agent only as the pairing the table lists", async () => {
  ...
  const listed = await dispatchAct(
    plugin,
    "spawnAgent",
    admittedForTest({
      providerSessionId: "session-idle",
  ...
  for (const stored of [
    ...
    const mismatched = await dispatchAct(
      plugin,
      "spawnAgent",
      admittedForTest({
        providerSessionId: "session-idle",
...
test("refuses to start an agent the row never listed, before any request exists", async () => {
  ...
  const unobserved = await dispatchAct(
    plugin,
    "spawnAgent",
    admittedForTest({
      providerSessionId: "session-unseen",
