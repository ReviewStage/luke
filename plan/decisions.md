...
## 2026-09-13 22:49Z — #1327 merged (`0e2cb8d5`): the exchange is the service's; LUKE-201/202 staffed

LUKE-200 (E5-3) landed under Dean's GO: `voice/function.ts` passes `deploymentExchange(...)`; the desktop's `LiveSessionService`, `brainAgentLiveBrain`, and `conversationLiveRecord` are gone and `LiveSessionHolder` holds the session; the sessions route admits `session.instructions.append` and `session.close`, reads `session.activity`, and closes 1008 on anything else (LUKE-185 2a). `PRIVACY.md` moved with it. **Named consequence:** the launch greeting (#1204, merged 21:43Z the same day) lost its desktop half in the rebase; its service half stands; re-homing is LUKE-202 §1b.

**Decisions made from the plan, not escalated:** (1) LUKE-201 (2c, Dean's ruling of 2026-09-13) staffed on the worker that built E5-3; the stop becomes a `VOICE_SERVICE_FRAME` and `instructions.append` leaves the route's allowlist. (2) LUKE-202 §1 and §1b — the arrival and calendar beats and the launch greeting — re-homed as service frames of the same shape as 2c (desktop keeps the decision and the once-per-run record; the service keeps the words; one frame back marks the beat spoken). This widens the live contract by three frame types along the pattern Dean already ruled for the stop; recorded here so it is reviewable, and the PR is orchestrator-word. Sequenced after LUKE-201 on the same worker because both edit the contract golden and the relay. (3) LUKE-202 §3 — the announcement pause and the introduction hold carried on the heartbeat's existing `quietUntil`, no new field — staffed in parallel on a second worker.
...
## 2026-09-14 00:29Z — LUKE-202 §1/§1b service half pressed as #1390: `session.beat` and `session.spoken`

Contract addition, decided from the plan along the 2c pattern: two `VOICE_SERVICE_FRAME` members. `session.beat` is the desktop's ask, after the handshake, for one of the three build-fixed beats Luke says unprompted (the arrival beat, the calendar line, the launch greeting); it is a union by kind, one struct per kind, carrying only the bounded observed values that kind's script may mention — one working session's title and the talk key's label for the arrival, the account's first name for the greeting, nothing for the calendar line — so a value a kind does not mention is refused rather than ignored. `session.spoken` is the one frame the service sends the desktop in this vocabulary: a proactive turn was spoken to its end, by kind alone, so the once-per-install and once-per-run records stay the desktop's. The words stay the service's script; the decision stays the desktop's. The desktop half (#1391) follows, orchestrator-word because it moves `PRIVACY.md` and opens a muted session on the Mac's own decision again.


## 2026-09-14 00:55Z — Dean: remove BYOK voice (LUKE-205 filed and staffed); LUKE-202 §2 recommendation put to Dean

Dean, 2026-09-14: "Let's remove BYOK since there is no brain behind it now." Filed as LUKE-205 under LUKE-95 and staffed (survey first, split by seam, orchestrator-word where `PRIVACY.md` or root `AGENTS.md` move). This lands the plan's standing "Cloud only. Keyed mode is removed." The hosted vault's provider keys are untouched.

LUKE-202 §2 (a Mac with no call open never speaks a briefing since E5-3; the offer goes to the phone after the grace) — orchestrator's recommendation to Dean: restore the Mac's half as a deterministic desktop rule, the same shape the pre-flip `wanted` had: when the account's changes feed shows an open offer, and this Mac is present (input within two minutes, screen unlocked) and not quiet (no meeting, pause, or owed introduction), the desktop opens a muted session; the service's briefing look claims and speaks the offer; the holder's idle close ends the session. No new frame, no model output on the trigger, the two-minute phone grace unchanged. Awaiting Dean's word.
...
## 2026-09-14 06:25Z — LUKE-206 (G1b) complete: the Mac composes no brain

Seven PRs (#1418, #1420, #1421, #1424, #1425, #1428, #1431): nothing hears the local brain's runs; the Gateway service answers the node vocabulary alone; the local brain composition, its wiring, performer, children, publication, and transcripts are deleted; the panel draws no local wait; the session performer becomes the three opens a row press reaches; the app-action chain and the errand flight are deleted (Dean's ruling); `PRIVACY.md` and the README say nothing on the Mac holds a conversation, a working memory, or a workspace, and that the two things the app does at the runtime's ask are opening an address and running the Calendar helper. Named consequences: a workspace Luke creates at the developer's ask no longer opens itself on the Mac (root `AGENTS.md` carries no sentence claiming it does); "Luke changes a setting when asked" has no path until a design gives the hosted brain one — filed only if Dean asks. The orphaned Gateway names were removed after all (LUKE-209 landed both PRs), so the ruling to keep them as test vocabulary is moot.

With LUKE-205 (BYOK removed) and LUKE-202 (the beats, the holds, and the briefing session re-homed) also complete tonight, every LUKE-95 child is Done except LUKE-159 (the physical-notch note is the last item) and LUKE-183 (blocked on Vercel shipping eve/vercel).


## 2026-09-14 20:40Z — Lane H (LUKE-210): H0b and H0a ruled, and the "iOS follow-up" closed

Recorded 2026-09-15 with H8 (LUKE-221), the lane's docs PR. Two rulings by Dean on 2026-09-14, carried through the lane's PRs:

**H0b (LUKE-212), 20:14Z: typed asks are removed on the phone.** Luke is voice only on every device. The sessions route refuses any text of the device's choosing, so the phone's composer and keyboard button went with the cutover (H4, #1456) and no `BrainAskClient` was written; `PRIVACY.md`'s "There is no way to type to Luke; every ask is spoken" is the truth of every platform. `TURN_ORIGIN.typed`, `ASK_ORIGIN.TYPED`, `BRAIN_HOST_TURN.TYPED`, and `DEPLOYMENT_TURNS`' typed row have no producer on any platform; deleting the typed origin end to end is a separate web-side follow-up, not the lane's.

**H0a (LUKE-211), 20:40Z: A′.** There is no WebRTC for watchOS, and the GPT Live WebSocket transport takes the project key alone, which the wrist must never hold; so the service holds the GPT Live primary WebSocket for the watch on Luke's key and relays audio both ways over a third voice route, `/api/voice/audio` (H5b, #1453/#1454; the openings moved to `apps/web/server/voice/opening.ts`). One format carries both directions, fixed at startup: **PCM16 at 16 kHz** (`LIVE_DEFAULT_AUDIO_FORMAT`), the compromise between the route's 40 MiB byte budget and Luke's voice; changing it is a product decision. A primary socket has no attach, so a watch call ends when the function does, at the **800-second** cap, with a clear end state and the next press opening a new call rather than a reattach (H5c, #1460). The watch's session stands under its own device row, the service's exchange, the hosted brain, and the voice writer, exactly as the Mac's and the phone's. **The product decision this carries: on the audio route alone, the developer's voice and Luke's transit the service in both directions;** on the sessions route (a Mac's or a phone's WebRTC call) they never do. `PRIVACY.md` says so route by route in H8. `SPEAKING_PLATFORMS` stays `{ macos }` (H6, #1468): a standing phone or watch call claims and speaks a briefing as the session's device, a phone or watch merely present is pushed to at once.

**The "iOS follow-up" is closed.** `plan/tickets.md` §"Coordination with the GPT-Live rollout" item 3 ("the phone has the hosted brain, so it uses the same voice service and client delegation as the Mac; no second backend"), `rollout/common.md` §11 on `orchestration/gpt-live-rollout`, and F2's cancellation ("voice left on its current path until the GPT-Live rollout's iOS follow-up") all named this work and never ticketed it. Lane H (LUKE-210) is that follow-up, landed: H4 (#1455, #1456) the phone as a WebRTC peer on the sessions route; H5 (#1453, #1454, #1459, #1460) the watch on the audio route; H6 (#1468) briefings claimed by a standing phone or watch call; H7 (#1469, #1474) the legacy Realtime path, the remote mint, and the retired pace deleted from the phone, the watch, and the service, leaving only the desktop's two unused mint routes (LUKE-226 retires them); H8 (LUKE-221) the disclosure. H9 (LUKE-220) is Dean's manual device pass, the only Apple gate, since CI has no Xcode.
