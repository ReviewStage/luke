...
- The urgency labels in `session-display.ts` are generated by
  `design/generate-surface-shared.mjs` and may not be hand-edited. The output
  is gitignored; change the table in the script and `pnpm generate` writes it
  before every check, test, and build.
