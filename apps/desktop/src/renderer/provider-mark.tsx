...
/**
 * Conductor's letter mark, taken verbatim from the published brand kit at
 * https://www.conductor.build/brandkit. The mark is drawn in `currentColor` so
 * the stylesheet can hold the brand palette in one place.
 */
...
function ConductorMark(): React.JSX.Element {
  // Decorative: every session row prints the provider name beside the mark, so
  // announcing it again would only repeat what a reader already hears.
  return (
    <span className="provider-mark provider-mark-conductor">
      <svg viewBox="0 0 115 174" fill="currentColor" stroke="currentColor" aria-hidden="true">
...
// Keyed by the same provider id the adapter reports, so a mark cannot end up
// beside sessions from a different provider.
const providerMarks = new Map<string, () => React.JSX.Element>([
  [CREDENTIAL_PROVIDER_ID.CONDUCTOR, ConductorMark],
]);

/**
 * Renders a provider's own mark. Providers Luke has no licensed mark for render
 * nothing rather than a stand-in glyph, so no row claims a brand it lacks.
 */
export function ProviderMark({ providerId }: { providerId: string }): React.JSX.Element | null {
  const mark = providerMarks.get(providerId);
