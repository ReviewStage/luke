 ...
 * mid-read, and content that vanishes under a hand is worse than content that
 * lingers: the hold keeps exactly what the strip was showing until the pointer
 * moves away. It can start nothing — a pointer arriving over an empty strip
 * finds nothing to hold — so nothing dismissed before the hover began is ever
 * resurrected.
 */
...
/**
 * What the strip is showing, snapshotted whole, so a held frame is drawn
 * exactly as it was, its tone included.
 */
 ...
 * words. The box is offered only while its content is drawn; an invisible
