 ...
 * the whole run rather than standing down and coming back — so the second action
 * costs one flight and nothing else. Folding several actions into one flight with
 ...
 * leaving two mechanisms where one does. Two actions are also two actions, and each
...
/** What an action is holding back until Luke's tap lands on the control. */
...
/** One action waiting to be signed. */
export interface PendingErrand {
  /** Where it should be signed, best first. An action with none flies nowhere. */
  ...
  page?: SettingsView;
  /** Whether this action is what stood the panel up. */
  opening: boolean;
  /**
   * Whether the panel is only borrowed to show this action. A switch has to be
...
/** Every action one reply asked Luke to sign: the one in the air, then the rest. */
export interface ErrandRun {
  /** The action being signed right now, once it has been handed to the flight. */
  flying?: PendingErrand;
  /** The actions still waiting their turn, oldest first. */
...
/** Adds an action to the end of the run. */
export function armErrand(run: ErrandRun, pending: PendingErrand): ErrandRun {
  return { ...run, waiting: [...run.waiting, pending] };
}

/**
 * The next action to hand to the flight, and the run with it in the air. Nothing
...
/**
 * The flight is over. The action leaves the air, and anything it is somehow still
...
/**
 * Whether the panel is still the run's to put away once this action has been
 * taken up.
 *
 * A run signs its actions in turn under one panel, so the question is asked of
 * the run rather than of any single action. An action that borrows the panel to show
 * a switch claims it back only if it is the one that stood it up — a panel
 * that was already open is somewhere the developer had gone themselves. An action
