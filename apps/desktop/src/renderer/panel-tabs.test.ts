...
test("panel tabs wrap with horizontal arrows", () => {
  assert.equal(panelTabForKey(PANEL_TAB.SESSIONS, "ArrowRight"), PANEL_TAB.CONVERSATION);
  assert.equal(panelTabForKey(PANEL_TAB.CONVERSATION, "ArrowRight"), PANEL_TAB.SETTINGS);
  assert.equal(panelTabForKey(PANEL_TAB.SETTINGS, "ArrowLeft"), PANEL_TAB.CONVERSATION);
