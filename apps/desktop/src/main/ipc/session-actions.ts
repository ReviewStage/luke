...
import { ACTION_RESULT_STATUS, type WireValue } from "@sidecar/wire";
...
export interface SessionActionsIpcDependencies {
 ...
 * call directly.
 */
export function registerSessionActionsIpc(dependencies: SessionActionsIpcDependencies): void {
  const { ipcMain, trustedSender, performer } = dependencies;
  /** One invoked bridge entry, whose refusal is an answer of the action's own kind rather than a rejected promise. */
    ...
    registerBridgeEntry(
      ...
      async (_context: BridgeContext, ...received: unknown[]): Promise<TResult> => {
        try {
          ...
          return await action.action(...(received as TArguments));
        } catch (error) {
  ...
  registerAction<[SessionIdentity], SessionOpenResult>(BRIDGE.openSession, {
    action: (identity) => performer.openSession(identity),
  ...
  registerAction<[SessionIdentity], SessionOpenResult>(BRIDGE.openSessionChange, {
    action: (identity) => performer.openSessionChange(identity),
