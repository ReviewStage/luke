...
import { createSessionActPerformer, NodeAnswerLostError } from "../session-act-performer";
...
export function composeRuntimeHost(options: RuntimeHostOptions): RuntimeHost {
  ...
  const openExternalThroughNode = async (url: string): Promise<void> => {
    const result = await nodes.invoke(NODE_CAPABILITY.OPEN_EXTERNAL, { url });
  ...
  const runAppleCalendarHelper: AppleCalendarHelperRun = async (helperArguments, timeoutMs) => {
    const result = await nodes.invoke(NODE_CAPABILITY.APPLE_CALENDAR_HELPER, {
