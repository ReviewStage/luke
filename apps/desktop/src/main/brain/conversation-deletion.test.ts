...
import { submitBrainAsk } from "./ipc";
...
function repository(client: RuntimeStoreClient) {
  ...
  } satisfies BrainStateRepository & { refuse: boolean };
  return repo;
}
...
test("a Clear under a held model answer fences the brain and the thread before any wait, keeps the line accepted after the press, archives what stood, and the next ask sees none of the old words", async () => {
  const c = composed();
  ...
  assert.equal(agent.request(late)?.status, BRAIN_REQUEST_STATUS.CANCELLED);
...
test("a Clear whose rows the store will not remove answers refused, yet the old words reach no context, no window, and no later launch of the brain", async () => {
  const c = composed();
  const { agent, client } = await seeded(c);
  c.refuseErase(true);
  const pressedAt = c.tick();
  ...
  // The old checkpoint is gone from the database: the marker's successor stands, empty.
  assert.equal(c.standing()?.reset_cleared_at, pressedAt);
  assertNoneOf([OLD_COMPACTION], c.rows());
...
test("a Clear whose marker the disk refuses answers refused without touching the rows, and still fences every context; the next landed write replaces what the disk kept", async () => {
  const c = composed();
  const { agent, client } = await seeded(c);
  c.repo.refuse = true;
  const pressedAt = c.tick();
...
test("a credential rebuild landing while the deletion waits on the disk builds over the successor, never the old checkpoint, and a second Clear during the first is harmless", async () => {
  const c = composed();
  const { agent, client } = await seeded(c);
  await agent.stop();
  ...
  assertNoneOf(OLD_WORDS, c.rows());
  assert.equal(
    loadBrainEnvelope(c.database, MAIN_SESSION_KEY).state?.generationId,
    c.store.generationId(),
  );
