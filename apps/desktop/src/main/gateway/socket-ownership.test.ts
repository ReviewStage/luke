    ...
    assert.ok(
      (
        await desktop.gateway.call(GATEWAY_METHOD.NODE_REGISTER, {
          ...
          capabilities: [NODE_CAPABILITY.OPEN_EXTERNAL],
        })
      ).ok,
    );
    const epoch = f.receiver.begin();
    assert.equal(f.receiver.markReady(epoch), true);
    const served = await f.service.nodes.invoke(NODE_CAPABILITY.OPEN_EXTERNAL, {
      url: "https://a",
    });
    assert.equal(served.status, NODE_CAPABILITY_STATUS.OK);
    desktop.connection.close();
    await new Promise((resolve) => setTimeout(resolve, 20));
    const absent = await f.service.nodes.invoke(NODE_CAPABILITY.OPEN_EXTERNAL, {
      url: "https://b",
    });
