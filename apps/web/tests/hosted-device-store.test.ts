 ...
 */
function deviceDatabase(answers: { updatedRows: number; deletedRows: number }) {
...
test("a bare heartbeat moves only the instants, scoped by account and row", async () => {
  ...
  assert.equal(writes.length, 1);
  assert.deepEqual(writes[0]?.set, { lastSeenAt: NOW, updatedAt: NOW });
  assert.equal(writes[0]?.hasWhere, true);
});
...
test("a heartbeat carries presence, a new push address, or a cleared one", async () => {
  ...
  assert.deepEqual(presence.writes[0]?.set, {
    lastSeenAt: NOW,
    updatedAt: NOW,
  ...
  assert.deepEqual(retokened.writes[0]?.set, {
    pushToken: null,
    pushEnvironment: null,
    updatedAt: NOW,
  });
  ...
  assert.deepEqual(cleared.writes[0]?.set, {
    lastSeenAt: NOW,
    updatedAt: NOW,
    pushToken: null,
    pushEnvironment: null,
