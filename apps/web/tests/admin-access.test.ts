...
test("a signed-in account that is neither is refused", () => {
  assert.equal(
    isAdminViewer(
      viewer({ githubAccountIds: ["99999999"], email: "someone@example.com" }),
      NO_EMAILS,
    ),
    false,
  );
...
test("a GitHub id admits even when the email is not on the env list", () => {
  ...
  assert.equal(
    isAdminViewer(
      viewer({ githubAccountIds: [ADMIN_GITHUB_ACCOUNT.DEAN], email: "dean@work.com" }),
      admins,
    ),
    true,
  );
...
test("a blank or absent env value is an empty allowlist, not a wildcard", () => {
  ...
  assert.equal(adminEmailsFromEnv(",, ,").size, 0);
  assert.equal(
    isAdminViewer(viewer({ email: "anyone@example.com" }), adminEmailsFromEnv("")),
    false,
  );
});
