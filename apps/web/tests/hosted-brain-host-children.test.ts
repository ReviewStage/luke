...
import { Deferred, Duration, Effect, Fiber, Redacted, Result, Schedule, Schema } from "effect";
...
test.skipIf(database.anotherConnection === undefined)(
  ...
  async () => {
    ...
    const another = database.anotherConnection();
    ...
    // The drop runs on a second connection built for this test alone, closed with its scope.
    const dropped = await database.run(
      Effect.scoped(
        Effect.provide(dropChildConversation(fixture.userId, child.childId, at(0)), another),
      ),
    );
