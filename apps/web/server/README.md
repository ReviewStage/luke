...
that carried it. A bundle legitimately gaining an external is a one-line change
to that file, which is the review moment a widened graph otherwise lacks: one
value import once carried the eve package into every function and failed them
all at load while every other check stayed green. The eve package and any other
entry of `FORBIDDEN_FUNCTION_EXTERNALS` may appear in no bundle at all, and a
negative test proves the guard fails when a route reaches one.
