...
tick, so the tick's order is: forget the ineligible, purge, sweep the
briefings on offer, then per batch of four accounts the pass and then the
opening, each account under one deadline, and a batch started only while a
