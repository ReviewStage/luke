...
/// at the moment of the call, so an action is validated against the roster as
...
/// the same on both.
@MainActor
public struct VoiceActionContext {
    ...
    public let actionClient: ActionClient
    public let accessToken: () async throws -> String
    /// Counts an action a provider accepted, under its allowlisted name alone.
    public let count: (ProductSessionAction, _ providerId: String) -> Void
    ...
    public init(
        ...
        count: @escaping (ProductSessionAction, _ providerId: String) -> Void,
...
/// Each action is validated on the device against what the conversation was
/// shown, then either performed here — an open, or the list — or sent
/// to the hosted action endpoint that serves it, which re-observes and validates
...
public func dispatchVoiceToolCall(
    ...
    case .sendSessionMessage:
        return await carry(VoiceAsks.message(arguments, in: context.sessions), context, .messageSend) {
            ask, token in
            try await context.actionClient.sendMessage(
                accessToken: token,
    ...
    case .runSessionControl:
        return await carry(VoiceAsks.control(arguments, in: context.sessions), context, .controlRun) {
            ask, token in
            try await context.actionClient.executeControl(
                accessToken: token,
                providerId: ask.session.providerId,
                providerSessionId: ask.session.sessionId,
    ...
    case .addWorkspaceAgent:
        ...
        return await carry(
            ...
            try await context.actionClient.spawnAgent(
                accessToken: token,
                providerId: ask.session.providerId,
                providerSessionId: ask.session.sessionId,
                agent: ask.agent,
    ...
    case .renameWorkspace:
        return await carry(
            ...
            try await context.actionClient.renameWorkspace(
                accessToken: token,
                providerId: ask.session.providerId,
                providerSessionId: ask.session.sessionId,
                name: ask.name
    ...
    case .renameSession:
        return await carry(
            ...
            try await context.actionClient.renameSession(
                accessToken: token,
                providerId: ask.session.providerId,
                providerSessionId: ask.session.sessionId,
                name: ask.name
    ...
    case .createWorkspace:
        ...
        return await carry(ask, context, .workspaceCreate) { ask, token in
            let answer = try await context.actionClient.createWorkspace(
                accessToken: token,
...
/// Runs one validated session action against its hosted endpoint, counts an
/// accepted one, and refreshes the roster so the next call is validated
/// against what the action changed. The endpoint's own answer is what the model
/// reads: accepted, or rejected with the server's reason.
@MainActor
private func carry<Ask: VoiceProviderAsk, Answer: ActionAnswer>(
    _ validated: Result<Ask, VoiceAskRefusal>,
    _ context: VoiceActionContext,
    _ counted: ProductSessionAction,
