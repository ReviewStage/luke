...
final class ActionMessageAnswerWireTests: XCTestCase {
    func testAcceptedDecodesWithoutReason() throws {
        let data = Data(#"{"result":"accepted"}"#.utf8)
        let answer = try JSONDecoder().decode(ActionMessageAnswer.self, from: data)
    ...
    func testUnsupportedDecodesCorrectly() throws {
        let data = Data(#"{"result":"unsupported","reason":"Mobile message actions are not yet available for this provider."}"#.utf8)
        let answer = try JSONDecoder().decode(ActionMessageAnswer.self, from: data)
...
// MARK: - ActionClient HTTP behaviour tests
...
final class ActionClientSendMessageTests: XCTestCase {
    ...
    func testHitsCorrectPath() async throws {
        let stub = StubHTTPClient { request in
            XCTAssertTrue(request.url?.path.hasSuffix("api/actions/message") == true)
            return (jsonData(["result": "accepted"]), makeResponse(url: request.url!, status: 200))
        }
        let client = ActionClient(baseURL: base, http: stub)
    ...
    func testUnauthorizedThrows() async {
        ...
        do {
            ...
            XCTFail("Expected ActionClientError.unauthorized")
        } catch ActionClientError.unauthorized {
            // expected
        } catch {
    ...
    func testServerErrorPropagates() async {
        ...
        do {
            ...
            XCTFail("Expected ActionClientError.serverError")
        } catch ActionClientError.serverError(let status) {
...
final class ActionClientCreateWorkspaceTests: XCTestCase {
    private let base = URL(string: "https://example.com")!
...
final class ActionClientRowActionTests: XCTestCase {
    private let base = URL(string: "https://example.com")!
    ...
    func testExecuteControlHitsPathWithControlId() async throws {
        let stub = StubHTTPClient { request in
            XCTAssertTrue(request.url?.path.hasSuffix("api/actions/control") == true)
    ...
    func testSpawnAgentOmitsEmptyNameAndTask() async throws {
        let stub = StubHTTPClient { request in
            XCTAssertTrue(request.url?.path.hasSuffix("api/actions/agent") == true)
            let body = self.bodyJSON(request)
    ...
    func testRenameSessionHitsPathWithTrimmedName() async throws {
        let stub = StubHTTPClient { request in
            XCTAssertTrue(request.url?.path.hasSuffix("api/actions/rename-session") == true)
            let body = self.bodyJSON(request)
    ...
    func testRenameWorkspaceHitsPath() async throws {
        let stub = StubHTTPClient { request in
            XCTAssertTrue(request.url?.path.hasSuffix("api/actions/rename-workspace") == true)
            return (jsonData(["result": "accepted"]), makeResponse(url: request.url!, status: 200))
        }
        let client = ActionClient(baseURL: base, http: stub)
    ...
    func testControlUnauthorizedThrows() async {
        ...
        } catch let error as ActionClientError {
...
final class ActionClientCreateWorkspaceSelectionTests: XCTestCase {
    private let base = URL(string: "https://example.com")!
