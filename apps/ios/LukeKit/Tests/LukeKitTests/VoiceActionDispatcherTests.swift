...
final class VoiceActionDispatcherTests: XCTestCase {
    ...
    private func context(
        ...
        count: @escaping (ProductSessionAction, String) -> Void = { _, _ in },
        ...
        VoiceActionContext(
            ...
            defaults: defaults
                ?? WorkspaceCreationDefaults(
                    store: UserDefaults(suiteName: "VoiceActionDispatcherTests.\(UUID().uuidString)")!
                ),
            actionClient: ActionClient(baseURL: base, http: http),
    ...
    func testAcceptedCreationPersistsTheChosenAgentModelAndEffort() async {
        ...
        let http = StubHTTPClient { request in
            XCTAssertEqual(request.url?.path, "/api/actions/workspace")
            let body = try JSONSerialization.jsonObject(with: request.httpBody ?? Data()) as? [String: String]
            XCTAssertEqual(
                body,
                [
