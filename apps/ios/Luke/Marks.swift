...
// MARK: - Provider marks (currentColor, traced from @sidecar/surface)
...
/// change both copies when a provider updates its mark. Every one is a
/// single-colour glyph riding `foregroundStyle`, like the GitHub mark above.
struct ProviderMark: View {
    ...
    var body: some View {
        ...
        Canvas { ctx, size in
            ...
            for path in art.paths {
                ctx.fill(Path(cgPath(fromSVG: path)), with: .foreground)
            }
        }
    ...
    private static func art(for provider: VaultProviderID) -> MarkArt {
        switch provider {
        case .conductor:
            MarkArt(width: 115, height: 174, paths: [
        ...
        case .copilot:
            MarkArt(width: 24, height: 24, paths: [
        ...
        case .devin:
            MarkArt(width: 425, height: 425, paths: [
        ...
        case .replicas:
            MarkArt(width: 225, height: 300, paths: [
